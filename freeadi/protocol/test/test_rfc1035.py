...
class TestRFC1035(object):
    ...
    def test_multi_component(self):
        s = '\x03foo\x03bar\x00'
        assert rfc1035.decompress(s, 0) == ('foo.bar', 9)

    def test_pointer(self):
        s = '\x03foo\x00\xc0\x00'
        assert rfc1035.decompress(s, 5) == ('foo', 7)

    def test_forward_pointer(self):
        s = '\xc0\x02\x03foo\x00'
        assert rfc1035.decompress(s, 0) == ('foo', 2)

    def test_pointer_component(self):
        s = '\x03foo\x00\x03bar\xc0\x00'
        assert rfc1035.decompress(s, 5) == ('bar.foo', 11)
    ...
    def test_multi_string(self):
        ...
        assert rfc1035.decompress(s, 0) == ('foo', 5)
        assert rfc1035.decompress(s, 5) == ('bar', 10)

    def test_null(self):
        s = '\x00'
        assert rfc1035.decompress(s, 0) == ('', 1)
